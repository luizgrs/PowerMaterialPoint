...
const sets = {
    'material': {
        imageType: 'XmlSvg',
        setupButton: (button, name, version, category) => button.innerText = name,
        fetchIcon: (name, version) => fetch(`https://fonts.gstatic.com/s/i/materialiconssharp/${name}/v${version}/24px.svg`)
...
let currentSet = 'material',
    currentSetDb;

function init() {
    const iconsDatabaseLoadingPromise = fetch(`${currentSet}_icons.json`).then(response => response.json());
    ...
    Promise.all([officeReadyPromise, domContentLoadedPromise, iconsDatabaseLoadingPromise])
        .then(values => iconsDbLoaded(values[2]));
}
...
function iconsDbLoaded(iconsDatabase) {
    currentSetDb = iconsDatabase;

    $id('loading-icons').remove();
    
    Object.keys(iconsDatabase).forEach(categoryName => {
        const catDiv = document.createElement('div');
        catDiv.classList.add('icons-category');
        document.body.appendChild(catDiv);

        const icons = iconsDatabase[categoryName];
        let buttons;
        ...
        buttons.forEach(catDiv.appendChild, catDiv);

        catDiv.addEventListener('click', insertIconButtonClick);
    });
}
...
function createIconButton(iconCategory, iconName, iconDefinition) {
    ...
    sets[currentSet].setupButton(iconButton, iconName, iconDefinition, iconCategory);
...
function insertIconButtonClick(event) {
    if (event.target instanceof HTMLButtonElement) {
        ...
        sets[currentSet].fetchIcon(button.dataset.name, currentSetDb[button.dataset.category][button.dataset.name])
            .then(image => {
                Office.context.document.setSelectedDataAsync(image, {
                    coercionType: Office.CoercionType[sets[currentSet].imageType],
                    imageLeft: 50,
                    imageTop: 50,
                    imageWidth: 400
                });
